...
function BigCard({ article, onRemove, onArticleClick }: { article: BookmarkedArticle; onRemove: () => void; onArticleClick: (a: BookmarkedArticle) => void }) {
  return (
    ...
    >
      <div style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 18, padding: '14px 15px' }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 7, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 9, color: 'var(--t4)', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {article.source}
              </span>
              <div style={{ width: 2, height: 2, borderRadius: '50%', background: 'var(--border2)', flexShrink: 0 }} />
              <span style={{ fontSize: 9, color: 'var(--t4)' }}>{relTime(article.publishedAt)}</span>
              <span style={{ fontSize: 9, color: 'var(--t3)', background: 'var(--bg2)', border: '0.5px solid var(--border2)', borderRadius: 20, padding: '2px 7px' }}>
                {article.topic}
              </span>
            </div>
            <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif", fontSize: 16, fontWeight: 500, color: '#ffffff', lineHeight: 1.42, marginBottom: 6 }}>
              {article.title}
            </div>
            <div style={{ fontSize: 9, color: 'var(--t4)', marginBottom: 9 }}>
...
function HalfCard({ article, onRemove, onArticleClick }: { article: BookmarkedArticle; onRemove: () => void; onArticleClick: (a: BookmarkedArticle) => void }) {
  return (
    ...
    >
      <div style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 16, overflow: 'hidden', height: '100%' }}>
        ...
        <div style={{ padding: '10px 11px 11px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 5, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 9, color: 'var(--t4)', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              {article.source}
            </span>
            <span style={{ fontSize: 8, color: 'var(--t3)', background: 'var(--bg2)', border: '0.5px solid var(--border2)', borderRadius: 20, padding: '2px 6px' }}>
              {article.topic}
            </span>
          </div>
          <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif", fontSize: 13, fontWeight: 500, color: '#ffffff', lineHeight: 1.38, marginBottom: 7 }}>
            {article.title}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }} onClick={e => e.stopPropagation()}>
  ...
  if (bookmarks.length === 0) {
    return (
      <div style={{ padding: '22px 22px 0' }}>
        <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif", fontSize: 12, color: 'var(--t4)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 22 }}>
          Briefly
        </div>
